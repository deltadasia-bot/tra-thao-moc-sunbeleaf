    ...
    <div className={cn("flex flex-col", className)} {...props}>
      {subcategories.map((subcategory, index) => (
        <button
          key={subcategory.id}
          className={cn(
            "relative flex min-h-16 w-full items-center justify-between gap-1 bg-clip-padding px-3 py-2 text-left",
        ...
        >
          <span className="flex-1 break-words text-small leading-tight text-text-primary">
            {subcategory.name}
          </span>
